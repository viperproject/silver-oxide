use std::cell::Cell;
use ::std::{
    mem::ManuallyDrop,
    ops::{Deref, DerefMut},
    sync::Mutex,
};

use crate::{
    parse::{Ident, Type},
    // program::{FnSig, Globals, MemberData, MemberKind},
    vmir::{
        members::{MemberRef, Members}, Def, DefId, LocalDefId, Symbol
    },
    TiVec,
};

use super::*;

pub struct TyCtxt<'tcx>(Box<GlobalCtxt<'tcx>>);

impl<'tcx> Deref for TyCtxt<'tcx> {
    type Target = GlobalCtxt<'tcx>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl DerefMut for TyCtxt<'_> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

pub struct GlobalCtxt<'tcx> {
    pub interner: Interner<'tcx>,
    pub types: Types<'tcx>,
    // pub std: Std<'tcx>,
    // pub(crate) globals: Globals<'tcx>,
    pub(crate) members: Members<'tcx>,
}

impl<'tcx> TyCtxt<'tcx> {
    pub fn members_topo(&self) -> impl Iterator<Item = MemberRef> + '_ {
        self.members.topo.iter().copied()
    }

    // pub fn mk_member_vec<T>(&self, mut f: impl FnMut(LocalDefId) -> T) -> TiVec<LocalDefId, T> {
    //     self.globals
    //         .data
    //         .iter_enumerated()
    //         .map(|(did, _)| f(did))
    //         .collect()
    // }

    pub fn def_data<Id: Into<DefId>>(&self, id: Id) -> Def<'tcx> {
        let id = <Id as Into<DefId>>::into(id);
        match id.as_local() {
            Some(id) => self.members.local_defs[id],
            None => todo!(),//self.std.data(id),
        }
    }

    // pub fn is_field<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     matches!(self.data(id).kind, MemberKind::Field)
    // }

    // pub fn is_predicate<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     matches!(self.data(id).kind, MemberKind::Predicate)
    // }

    // pub fn is_function<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     matches!(self.data(id).kind, MemberKind::Function)
    // }

    // pub fn is_method<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     matches!(self.data(id).kind, MemberKind::Method)
    // }

    // pub fn is_axiom<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     matches!(self.data(id).kind, MemberKind::DomainAxiom)
    // }

    // pub fn is_heap_dependent<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     self.fn_sig(id).unwrap().heap_dependent
    // }

    // pub fn is_preconditionless<Id: Into<DefId>>(&self, id: Id) -> bool {
    //     self.data(id).sig.unwrap().preconditionless
    // }

    // pub fn fn_sig<Id: Into<DefId>>(&self, id: Id) -> Option<&FnSig<'tcx>> {
    //     let id = <Id as Into<DefId>>::into(id);
    //     match id.as_local() {
    //         Some(id) => self.globals.sigs.sigs[id].as_ref(),
    //         None => self.std.fn_sig(id),
    //     }
    // }

    // pub fn item_name<Id: Into<DefId>>(&self, id: Id) -> Option<Symbol<'tcx>> {
    //     self.data(id).sig.map(|s| s.name)
    // }

    // pub fn member(&self, id: LocalDefId) -> &Member<'tcx> {
    //     &self.members.bodies[id]
    // }

    // pub fn global_ref(&self, symbol: Symbol<'tcx>) -> Option<DefId> {
    //     self.std
    //         .global_ref(symbol)
    //         .or_else(|| self.globals.resolved.get(&symbol).map(|id| id.into()))
    // }

    /// Turn into an unsafe static, only to be used when stored e.g. in an egraph
    pub unsafe fn as_static(
        &'tcx self,
    ) -> (
        &'static TyCtxt<'static>,
        core::marker::PhantomData<&'tcx Self>,
    ) {
        let tcx = core::mem::transmute::<&'tcx TyCtxt<'tcx>, &'static TyCtxt<'static>>(self);
        (tcx, core::marker::PhantomData)
    }

    // pub(crate) fn resolve_global_ref(&self, ident: &Ident) -> DefId {
    //     let ident = self.interner.mk_symbol(ident);
    //     self.global_ref(ident).unwrap()
    // }

    pub(crate) fn const_ty(&self, const_: Const<'tcx>) -> Ty<'tcx> {
        use crate::parse::ConstKind::*;
        match const_.kind() {
            Bool(..) => self.types.bool_,
            Int(..) => self.types.int_,
            Real(..) => self.types.real_,
            Null => self.types.ref_,
            Epsilon => self.types.real_,
            Wildcard => self.types.real_,
            Heap(..) => self.types.heap_,
        }
    }

    /// Only for use in development features (e.g. printing DefId)
    pub(crate) unsafe fn global_ref_unchecked() -> impl Deref<Target = TyCtxtCopy<'static>> {
        TCX.lock().unwrap()
    }
}

pub(crate) struct TyCtxtCopy<'tcx>(Option<ManuallyDrop<TyCtxt<'tcx>>>);
unsafe impl Send for TyCtxtCopy<'static> {}

impl<'tcx> Deref for TyCtxtCopy<'tcx> {
    type Target = TyCtxt<'tcx>;
    fn deref(&self) -> &Self::Target {
        self.0.as_ref().unwrap()
    }
}

static TCX: Mutex<TyCtxtCopy<'static>> = Mutex::new(TyCtxtCopy(None));

impl<'tcx> Default for TyCtxt<'tcx> {
    fn default() -> Self {
        let interner = Interner::default();
        let types = Types::new(&interner);
        // let std = Std::new(&interner);
        let gcx = GlobalCtxt {
            interner,
            types,
            // std,
            // globals: Default::default(),
            members: Default::default(),
        };
        let tcx = TyCtxt(Box::new(gcx));

        let tcx_copy = unsafe { core::mem::transmute::<&TyCtxt<'tcx>, &TyCtxt<'static>>(&tcx) };
        let tcx_copy = unsafe { ManuallyDrop::new(core::ptr::read(tcx_copy)) };
        TCX.lock().unwrap().0.replace(tcx_copy);

        tcx
    }
}
