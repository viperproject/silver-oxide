use crate::{
    parse::{self, AssignRhs, ConstKind, ExpKind, StarOrNames, UnOp},
    program::{
        body::{
            Block, BlockConds, Body, FoldUnfold, InhaleExhale, Operand, OperandKind, Statement,
            StatementKind,
        },
        exp::{Exp, ExpOperand, ExpOperandKind},
        resource::ResourceExp,
        ArgRef, BasicBlock, DefId, Local, MemberKind, Ty, TyKind,
    },
    HashMap,
};
...
impl<'tcx> TranslationCtxt<'_, 'tcx> {
    pub(crate) fn translate_body(mut self, body: &parse::StmtBlock) -> Body<'tcx> {
        ...
        for (bb, data) in cfg.preorder() {
            ...
            if let Some(lh) = data.as_loop_head() {
                ...
                let kind = StatementKind::Eval(
                    tmp,
                    Exp::new_use(
                        ExpOperandKind::Local(self.curr_heap()),
            ...
            if let Some(lh) = data.back_edge_to() {
                ...
                let false_ =
                    ExpOperandKind::Const(self.tcx.interner.mk_const(ConstKind::bool(false)));
        ...
        match rhs {
                ...
                match tgt {
                    Ok(local) => {
                        block.stmts.push(Statement {
                            kind: StatementKind::Eval(local, value),
                        });
                    }
                    Err(rcv) => {
                        if let Some(nd) = value.as_operand() {
                            block.stmts.push(Statement {
                                kind: StatementKind::Assign(self.curr_heap(), rcv, nd),
                            });
                        } else {
                            let tmp = self.locals.push_and_get_key(ty);
                            block.stmts.push(Statement {
                                kind: StatementKind::Eval(tmp, value),
                            });
                            let tmp = Operand {
                                ty,
                                kind: OperandKind::Local(tmp),
                            };
                            block.stmts.push(Statement {
                                kind: StatementKind::Assign(self.curr_heap(), rcv, tmp),
                            });
                        }
                    }
                }
            ...
            AssignRhs::Call(ident, args) => {
                    ...
                    .map(|(tgt, ety)| {
                        ...
                        tgt.unwrap_or_else(|nd| {
                            ...
                            post_call.push(Statement {
                                kind: StatementKind::Assign(self.curr_heap(), nd, tmp_nd),
                            });
                            tmp
                        })
                    })
                ...
                let heap = Operand {
                    ...
                    kind: OperandKind::Local(self.params[&ArgRef::Heap(None)]),
                };
                let args = args
                    .iter()
                    .zip(sig.caller_args().1)
                    .map(|(arg, ty)| self.translate_exp_local(arg, *ty, block))
                    .chain([heap])
                    .collect();
                block.stmts.push(Statement {
    ...
    fn translate_inhale_new(&mut self, tmp: Local, new: &StarOrNames, block: &mut Block<'tcx>) {
        let fields: Vec<_> = match new {
            ...
            StarOrNames::Names(idents) => idents
                .iter()
                .map(|ident| self.tcx.resolve_global_ref(ident))
                .collect(),
        };
        let tmp = ExpOperand {
            ty: self.locals[tmp],
            kind: ExpOperandKind::Local(tmp),
        };
    ...
    fn translate_exp_local(
        ...
        exp.as_operand().unwrap_or_else(|| {
            ...
            block.stmts.push(Statement {
                kind: StatementKind::Eval(local, exp),
            });
            Operand {
                ty,
