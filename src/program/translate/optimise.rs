...
use crate::program::{
    exp::{ExpLine, ExpLineKind, ExpOperand},
    Const,
};
...
impl<'tcx> ExpTranslator<'_, '_, 'tcx> {
    pub(super) fn optimise_line(
        &mut self,
        mut line: ExpLine<'tcx>,
    ) -> Result<ExpOperand<'tcx>, ExpLine<'tcx>> {
        use ExpLineKind::*;
        match &mut line.kind {
            Use(op) => Ok(*op),
            Ternary(
                [ExpOperand {
                    kind: ExpOperandKind::Const(_),
                    ..
                }, ..],
            ) => unreachable!(),
            Ternary([c, t, e]) => match (
                ...
                (Some(b @ true), Some(true)) | (Some(b @ false), Some(false)) => {
                    let kind =
                        ExpOperandKind::Const(self.tcx.tcx.interner.mk_const(ConstKind::bool(b)));
                    Ok(ExpOperand {
            ...
            UnOp(
                op,
                ExpOperand {
                    kind: ExpOperandKind::Const(c),
                    ..
                },
            ) => self
                .optimise_unop(*op, *c)
                .map(ExpOperandKind::Const)
                .map(|kind| ExpOperand { ty: line.ty, kind })
                .ok_or(line),
            BinOp(
                op,
                [ExpOperand {
                    kind: ExpOperandKind::Const(lhs),
                    ..
                }, ExpOperand {
                    kind: ExpOperandKind::Const(rhs),
                    ..
                }],
            ) => self
                .optimise_binop(*op, *lhs, *rhs)
                .map(ExpOperandKind::Const)
                .map(|kind| ExpOperand { ty: line.ty, kind })
                .ok_or(line),
            // TODO?
        ...
        match op {
            ...
            Iff => Some(
                self.tcx
                    .tcx
                    .interner
                    .mk_const(Bool(lhs.as_bool() == rhs.as_bool())),
            ),
            Eq => match (lhs.kind(), rhs.kind()) {
                (Wildcard, Real(r)) | (Real(r), Wildcard) => (*r <= ConstKind::rational(0))
                    .then(|| self.tcx.tcx.interner.mk_const(Bool(false))),
            ...
            Lt => match (for_compare(lhs), for_compare(rhs)) {
                (Int(l), Int(r)) => Some(self.tcx.tcx.interner.mk_const(Bool(l < r))),
                (Real(l), Real(r)) => Some(self.tcx.tcx.interner.mk_const(Bool(l < r))),
                (Wildcard, Real(r)) => (*r <= ConstKind::rational(0))
                    .then(|| self.tcx.tcx.interner.mk_const(Bool(false))),
                (Real(r), Wildcard) => (*r <= ConstKind::rational(0))
                    .then(|| self.tcx.tcx.interner.mk_const(Bool(true))),
                _ => None,
            },
            Le => {
    ...
    pub(super) fn get_condition(&self, kind: &ExpLineKind) -> Option<Box<[ExpCond<'tcx>]>> {
        let cless = match kind {
            ...
            ExpLineKind::Call(did, _) => matches!(
                self.tcx.tcx.data(*did).kind,
                MemberKind::DomainFunction | MemberKind::Predicate | MemberKind::Field
            ),
            _ => false,
        };
        let cless = cless
            && kind.operands().iter().all(|o| {
                o.as_exp_local()
