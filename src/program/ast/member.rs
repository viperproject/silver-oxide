...
use crate::{parse::{Declaration, DomainElementKind, Program}, program::{body::StatementKind, translate::TranslationCtxt, CanDot, DefId, Ty, TyCtxt, TyKind}, HashMap, HashSet, TiVec};

use super::{body::{Body, Statement}, exp::{Exp, ExpLine, ExpLineKind}, idx::LocalDefId, resource::ResourceExp};
...
impl<'tcx> Members<'tcx> {
    ...
    fn calculate_member(&self, tcx: &TyCtxt<'tcx>, id: LocalDefId, decl: &Declaration) -> Member<'tcx> {
        ...
        match decl {
            ...
            DomainElement(crate::parse::DomainElement { domain, kind: DomainElementKind::Axiom(ax) }) => {
            ...
            Function(f) => {
                ...
                trcx.add_return();
                let post = trcx.translate_exp(&f.contract.postcondition.exp, tcx.types.bool_, have_heap);
    ...
    fn calculate_member_deps(&mut self, tcx: &TyCtxt<'tcx>, id: LocalDefId) {
        let this = |kind| DepNode { id: id.into(), kind };
        match &self.bodies[id] {
            ...
            Member::DomainAxiom(domain, ..) => {
                ...
                self.dep_graph.add_edge(this(false), DepNode { id: domain.into(), kind: false }, ());
            }
            Member::Field => (),
    ...
    fn calculate_topo(&mut self, tcx: &TyCtxt<'tcx>) {
        ...
        let tr_dep = |mut from: DepNode, to: DepNode| {
            ...
            if tr_dep {
                let (from, to) = (tcx.item_name(from.id).unwrap(), tcx.item_name(to.id).unwrap());
        ...
        for &node in &topo {
            ...
            let fn_pre = DepNode { id: node.id, kind: false };
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    fn walk_lines<'a>(&self, dep_graph: &mut DepGraph, this: DepNode, lines: impl Iterator<Item = &'a ExpLine<'tcx>>) where 'tcx: 'a {
        for line in lines {
            self.walk_tys(dep_graph, this, line.ty.walk());
            match line.kind {
                ExpLineKind::Call(did, ..) => {
                    if self.is_function(did) {
                        let did = DepNode { id: did.into(), kind: false };
    ...
    fn walk_tys(&self, dep_graph: &mut DepGraph, this: DepNode, tys: impl Iterator<Item = Ty<'tcx>>) {
        for ty in tys {
            if let TyKind::Domain(domain, ..) = *ty.kind() {
                let domain = DepNode { id: domain, kind: false };
                dep_graph.add_edge(domain, this, ());
            }
        }
    }

    fn walk_stmts<'a>(&self, dep_graph: &mut DepGraph, this: DepNode, stmts: impl Iterator<Item = &'a Statement<'tcx>>) where 'tcx: 'a {
        for stmt in stmts {
            use StatementKind::*;
            match &stmt.kind {
                ...
                Call(_, method, _) => {
                    dep_graph.add_edge(DepNode { id: *method, kind: false }, this, ());
                }
    ...
    fn walk_fold_unfold(&self, dep_graph: &mut DepGraph, this: DepNode, loc: Ty<'tcx>) {
        let TyKind::ResourceId(ty) = loc.kind() else { unreachable!() };
        if let TyKind::Compound(cid) = ty.kind() {
            assert!(cid.contract.is_none());
            let cid = DepNode { id: cid.did, kind: false };
