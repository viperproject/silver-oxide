use crate::vmir::middle::{FunctionId, MethodId, ResourceId};
use crate::vmir::ty::{AdtDef, AdtDefData, DomainKind, Interner, Ty, TyKind, TyList, VariantDef};
use crate::vmir::{DefId, LocalDefId, Symbol};
use crate::HashMap;
...
impl<'tcx> Globals<'tcx> {
    ...
    fn calculate_kind(
        ...
        let mut sig = decl.signature().map(|sig| DeclSig {
            ...
            rets: sig.ret.len(),
            preconditionless: decl.contract().is_none_or(|c| c.precondition.is_true()),
        });
        let kind = match decl {
...
//     pub pre: Result<FunctionId, ResourceId>,
//     pub post: FunctionId,
//     pub this: FunctionId,
// }
...
pub struct DeclSig<'tcx> {
    ...
    pub rets: usize,
    pub preconditionless: bool,
}

#[derive(Debug, Default)]
pub struct GlobalSigs<'tcx> {
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    fn calculate_fn_sig(&mut self, id: LocalDefId, decl: &Declaration) -> Option<FnSig<'tcx>> {
        ...
        let mk_compound = |contract| {
            let fields = todo!();
            let adt = self.interner.mk_adt_def(AdtDefData {
                id: id.into(),
                variants: [VariantDef { fields }].into_iter().collect(),
            });
            let ret = TyKind::Domain(DomainKind::Adt(adt), TyList::empty());
            self.interner.mk_ty_from_kind(ret)
        };
        let mk_resource_id = |ty| {
            let ret = TyKind::Address(ty);
            self.interner.mk_ty_from_kind(ret)
        };
